...
export function getCommandIcon(commandLabel, size = 'small') {
  const commands = {
    ...
    'Win': { bgColor: '#b084cc', icon: 'sparkles' },
    'Game 1': { bgColor: '#658ea9', icon: '1' },
    'Game 2': { bgColor: '#d7a449', icon: '2' },
  ...
  if (cmd.icon === '1' || cmd.icon === '2') {
    ...
    div.appendChild(createIcon(cmd.icon, `${isSmall ? 'w-4 h-4' : 'w-8 h-8'} text-white`));
  }
...
export function getDeviceIcon(type, size = 'medium') {
  ...
  div.className = `${sizes[size]} rounded-full flex items-center justify-center flex-shrink-0 ${
    type === 'extension' ? 'bg-gray-600' : 'bg-gray-500'
  }`;
...
export function getSignalIcon(signal) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    bar.className = `w-1 ${i === 0 ? 'h-2' : i === 1 ? 'h-3' : 'h-4'} rounded-sm ${
