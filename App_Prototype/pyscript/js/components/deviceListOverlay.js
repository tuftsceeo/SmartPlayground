...
export function createDeviceListOverlay(devices, range, isRefreshing, editingDeviceId, nicknames, onClose, onRangeChange, onRefresh, onStartEdit, onSaveNickname) {
  ...
  overlay.innerHTML = `
    <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
      ...
      <button class="ml-auto w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors ${isRefreshing ? 'animate-spin' : ''}" id="refreshBtn">
  ...
  if (devices.length === 0) {
    ...
    devices.forEach(device => {
      ...
      card.className = 'px-4 py-3 border-b border-gray-100 hover:bg-gray-50 flex items-center gap-3';
      ...
      card.innerHTML += `
        ...
        <div class="flex items-center gap-3" id="status-${device.id}">
          <button class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-200 transition-colors" id="edit-btn-${device.id}">
