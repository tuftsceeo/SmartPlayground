...
LED_PIN = 1                     # <-- CHANGE THIS to your actual pin
led = Pin(LED_PIN, Pin.OUT)      # initialize once
...
#central writes to Rx and Peripheral writes to Tx
...
def handle_cmd(data: bytes):
    # data is the raw bytes the browser wrote to the UART RX characteristic
    try:
        s = data.decode().strip()
        # accept a couple of spellings; match whatever you send from JS
        if s in ("LED 1", "ON", "LIGHT ON"):
            led.value(1)
        elif s in ("LED 0", "OFF", "LIGHT OFF"):
            led.value(0)
        # (optional) echo back a status over TX so the web page can log it
        # If you want to echo: p.send(("ACK: " + s).encode())
    except Exception as e:
        print("Parse error:", e)

#-------------------Peripheral---------------------------------------------------------------------------------------------------------------                
class Yell(Useful): 
    def __init__(self, name = 'Pico', interval_us=100000, verbose = True, type = 'uart'):
        self.setup(name, verbose, self._irq)
...
def main(mode = 'P'):
    if mode == 'P':
        try:
            ...
            # hook our handler so incoming writes toggle the LED
            p._write_callback = handle_cmd

            if p.connect_up():
                print('P connected')
                # (optional) send something once connected
                time.sleep(0.5)
                p.send(b"ready")
                # keep the peripheral running; you can remove the payload loop
                while p.is_connected:
                    time.sleep(1)
                    # if you still want to read buffered text (from your default rx),
                    # you can, but our handler already acts on writes.
        except Exception as e:
            print(e)
        finally:
            p.disconnect()
