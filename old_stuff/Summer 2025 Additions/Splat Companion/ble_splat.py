...
class OpenSplat():
    ...
    def decode_button(self,value):
        ...
        if(value & 0b1000 or value & 0b0100 or value & 0b0010 or value & 0b0001):
             self.playSound(config.sound,config.volume)
...
def splat_BLE():
    ...
    splat = OpenSplat("AB:42:00:00:1E:7B")  # Use your Splat's MAC address
    splat.connect(timeout = 5)
    splat.readSwitches()
    try:
        if splat.connect(timeout= 5):
