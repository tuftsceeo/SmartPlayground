...
class Notes(Game):
    ...
    async def loop(self):
        """
        Async task to play a random note while button is pressed.  You can choose the note by selecting an NFC card
        Stops when self.running is set to False.
        """

        if self.main.topic == '/nfc':
            self.main.topic = '/notify'
            num = self.main.value
            self.note = list(NOTES.keys())[num]
            self.frequency = NOTES[self.note]
            self.main.log_message(f"You were assigned {self.note} at a frequency of {self.frequency}.")
