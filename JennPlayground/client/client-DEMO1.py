...
def draw_bar(screen, number):
    ...
    text = font_large.render('Motion Level', True, GRAY)
    ...
    for i in range(10):
        if i < number:
            #Green Bars
            pygame.draw.rect(screen, GREEN, (i * int(bar_width * bar_spacing), screen_height // 2, bar_width, screen_height // 2))
        else:
            #Gray Bars
            pygame.draw.rect(screen, GRAY, (i * int(bar_width * bar_spacing), screen_height // 2, bar_width, screen_height // 2))
...
async def handle_client(host, port, screen):
    ...
    try:
        ...
        while True:
            ...
            # Convert bytes to string and then to integer
            motion_level = int(data.decode())

            # Draw the motion level bar chart
            draw_bar(screen, motion_level)
