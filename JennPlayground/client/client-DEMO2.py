...
def draw_bar(screen, quads):
    screen.fill(BACKGROUND)  # Clear the screen
    bar_width = screen_width // (bar_spacing*2)
    bar_height = screen_height // (bar_spacing*2)
    gap = screen_height // (2) - screen_height // (bar_spacing*2) 
    
    for i in range(2):
        for j in range(2):
            # Calculate the position and color based on the binary array
            bar_x = int(j * bar_width * bar_spacing)
            bar_y = int(i * bar_height * bar_spacing)
            color = GREEN if quads[i * 2 + j] == 1 else GRAY

            # Draw the cell
            pygame.draw.rect(screen, color, (bar_x, bar_y+gap, bar_width, bar_height))

    draw_exit_button(screen)  # Draw the exit button
    text = font_large.render('Motion Quads', True, GRAY)
...
async def handle_client(host, port, screen):
    ...
    try:
        ...
        while True:
            ...
            quadstring = data.decode()
            quads = [int(x) for x in quadstring]

            # Draw the motion level bar chart
            draw_bar(screen, quads)
