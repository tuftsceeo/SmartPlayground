...
async def capture_and_adjust_gain():
    ...
    while True:
        if runs % update_gap == 0:  # Update settings once every update_gap "good" frames
            ...
            if runs == 1 and (not motion_triggered):
...
def find_nearest_blob(blob, previous_blobs):
    ...
    if nearest_blob:
        ...
        good_area = True # nearest_blob.area()*.25<blob.area() and blob.area()<nearest_blob.area()*2
...
async def perform_computations():
    ...
    while True:
        ...
        if new_frame:
            ...
            print("END", clock.fps(), "Move: ", movement, white_pixels, "Reset Trig: ",  bad_blob_counter)
