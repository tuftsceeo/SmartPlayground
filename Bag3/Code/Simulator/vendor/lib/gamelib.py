...
class Device:
    ...
    def begin(self, slug):
        ...
        self.slug = slug
        self.nfc_every = NFC_EVERY
        # No module for the slug when a game is run straight from the REPL or
        # the simulator rather than launched by a card; it has no role either.
        self.role = game_store.role_of(module) if module else None
        self._events = []
        self._exit = None
        self._passes = 0
        names = self.card_commands()
        names.discard(slug)
        self._exit_names = names
        self._last_uid = None
