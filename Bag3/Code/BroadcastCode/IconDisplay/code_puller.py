...
The icon display's copy. Identical in wire behaviour to MockWand's; main.py
is what differs, passing hubtype="icon_display" and an icon_dir so the Box
serves the display's file and its named icons rather than the wand's.

Calls enow.shutdown() before WiFi connect. See EXTERNAL_ANTENNA below for the
GPIO 3/14 antenna switch -- it is off by default, matching the prototype.
"""
...
# PEER: BBoxFirmware/code_server.py and MockWand/code_puller.py each hold a
# hand-kept copy of SSID/PWD/PORT/CHUNK/YIELD_MS and of the wire protocol in
# pull() below. There is no shared module (they run on different devices), so
...
def _reset_sta(external_antenna, verbose):
    ...
    associate with it. This device selects the onboard antenna (it has
    nothing on its u.FL connector), but "onboard" still has to be driven,
    not left floating.
