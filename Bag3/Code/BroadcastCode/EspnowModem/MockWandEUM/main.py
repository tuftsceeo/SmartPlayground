...
# See Bag3/Code/BroadcastCode/BroadcastBox/docs_and_design/old/2026-09-01-wifi-handoff-diagnosis.md.
#
...
# ─────────────────────────────────────────────
# ─────────────────────────────────────────────
# ESP-NOW CODE PULL (EspnowModem copy only)
# ─────────────────────────────────────────────
# getcode fetches the game over the ESP-NOW link that is already up (see
# lib/espnow_code.py and EspnowModem/host/code_sender.py) instead of queueing
# a WiFi pull and resetting. False restores the pull_flag/reset/code_puller
# path unchanged.
CODE_VIA_ESPNOW = True
# BENCH: a broadcast {"type":"getcode","slug":s} from the host acts like a
# getcode tap, so transfers can be repeated without a card.
REMOTE_GETCODE = True
...
def _espnow_pull_and_launch(enow, slug, nfc, accel, i2c, batt):
    ...
    buz.success()
    time.sleep_ms(600)
    name = stats.get("name") or slug
    # A previous copy of this game may still be imported; drop it so the
    # new file is what runs.
    _unload_game(name)
        ...
        try:
            ...
            if nfc_sleeping:
                ...
                if result == "stop":
                    ...
                    show_idle(last_soc, 0)
                    last_uid = None
                elif isinstance(result, tuple) and result[0] == "getcode":
                    _clear_rules_state(enow)
                    rules = {}; editing = None; pending_combinator = None
                    buz.stop()
            ...
            if head == "getcode" and CODE_VIA_ESPNOW and enow.is_active:
                _clear_rules_state(enow)
                rules = {}; editing = None; pending_combinator = None
                buz.stop()
