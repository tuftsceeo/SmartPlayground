# PEER: Bag3/Code/BroadcastCode/BroadcastBox/BBoxFirmware/code_server.py — keep in sync.
# The two are the same server on different boards; only the comments about
# each board's heap, and the Dial-only prewarm_ap() below, differ. Fix one
# copy and say which (Bag3/AGENTS.md).
"""
...
def prewarm_ap():
    ...
    """
    # gc.mem_free() alone cannot answer what this function is for. The AP
    # needs one large contiguous IDF block, so idf_largest is the number
    # that says whether the reservation happened and whether it survived
    # active(False). See idf_heap().
    print("# AP prewarm: before gc_free=%d idf_free=%s idf_largest=%s"
          % ((gc.mem_free(),) + idf_heap()))
    ap = network.WLAN(network.AP_IF)
    ap.active(True)
    print("# AP prewarm: after active(True) gc_free=%d idf_free=%s idf_largest=%s"
          % ((gc.mem_free(),) + idf_heap()))
    ap.active(False)
    gc.collect()
    print("# AP prewarm: after active(False)+gc gc_free=%d idf_free=%s idf_largest=%s"
          % ((gc.mem_free(),) + idf_heap()))
    ...
    def _step_body(self, c):
        if c.outbuf is None:
            ...
            if c.chunkbuf is None:
                ...
                # path of a device that logs "low mem, deferring accept" at
                # ~29 kB free during back-to-back pulls; a slow client spans
                # far more of that churn than a fast one.
