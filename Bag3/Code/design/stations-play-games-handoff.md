...
- **Coding station** — `Bag2/Code/Stations/Programming Station/` (`main.py`, `hubtype.txt`
  reading `programming_station`, `README.md`). The name differs; this is the device described
  as the Coding Station. Four PN532 readers share one I2C bus through a **PCA9546 mux**
  (the PN532's address is fixed at 0x24, so only one channel is live at a time); an operator
  places up to four colour tags in the slots, presses the button on GPIO0, and the sequence is
  broadcast over ESP-NOW. This is how a round of Color Quest begins. An 18-LED strip blinks the
  scanned colours back as confirmation. No Bag 3 implementation.
  Note the Bag 3 concept is selector *wheels* rather than tag slots — same abstract
