/**
 * deviceLink.js -- the USB link to an Icon Display Station, vendored from
 * the station's own webapp (Stations/Icon Display Station/webapp/js/device).
 * It owns the serial connection lifecycle and forwards icon commands to
 * NdjsonLink once the device is heard from. ChatBroadcast uses it to send a
 * game's station role file and its icons; it does not install firmware.
 *
 ...
 *
 * NdjsonLink is DETACHED before any raw-REPL work (uploadFile) and
...
export class DeviceLink {
  ...
  /**
   * Write one file to the station's flash over the raw REPL.
   *
   * Used for a game's role file, which the JSON command set has no verb
   * for. The JSON link is detached for the duration: NdjsonLink clears the
   * adapter backlog on every chunk, which is the same backlog the raw REPL
   * reads its replies from, so the two cannot run at once. It is always
   * re-attached, including after a failure -- leaving it detached makes the
   * app deaf to every later hello.
   *
   * @param {string} path    absolute path on the station, e.g. /games/x.py
   * @param {string} content the file's text
   */
  async uploadFile(path, content) {
    this._detachJson();
    try {
      await this.repl.enterRepl();
      await this.repl.enterRawRepl();
      const dir = path.slice(0, path.lastIndexOf("/"));
      if (dir) await this.repl.ensureDirectory(dir);
      await this.repl.uploadFile(path, content);
      await this.repl.exitRawRepl();
      await this.repl.softReset();
      const { found } = await this.adapter.readUntil('"type":"hello"', 10000);
      if (!found) {
        throw new Error(
          `${path} was written, but the station did not announce itself after a restart. ` +
            "Power-cycle the board and reconnect."
        );
      }
    } finally {
      this._attachJson();
