"""
nfc_reader.py -- read a command from an NFC card.

Cards carry plain NDEF text: a game module name, a control tag ("stop",
...
class NfcReader:
    ...
    def _match_prefixed(self, text):
        """Return the full text for a valid "<prefix>:<module>" card, else None.

        The tail must be a legal module name because it doubles as the module
        of a pulled role (/games/<module>.py, imported by name). Rejecting
        anything else here keeps a malformed card from ever reaching the
        filesystem or __import__.
        """
        ...
        if head not in self.prefixes:
            return None
        if tail and not is_valid_module(tail):
            return None
        return text
...
# Backward-compatible alias for code that imported the opcode-era name.
