...
# ---- UART wiring -----------------------------------------------------
# Board: Seeed XIAO ESP32-C6. (Briefly flip-flopped with the C3 pin
# mapping during Gate A -- the two XIAO boards are visually identical and
# the wrong one was on the bench; the C3's boot banner and its
# ValueError: invalid pin for GPIO16/17 were the tell. Confirmed the
# actual board's boot banner says "ESP32C6" before trusting these pins.)
# Sensor wired to the pins silkscreened D6/D7, which on the C6 are
# GPIO16/GPIO17 -- the SoC's default UART0 pins ("U0TXD"/"U0RXD" per
...
# with a system use of it. tx=16 is the SoC's own TX (drives LD2450 RX);
# rx=17 is the SoC's own RX (reads LD2450 TX) -- i.e. wires cross between
# the two boards as usual.
UART_ID = 1
UART_TX = 16
UART_RX = 17
UART_BAUD = 256000
