...
A 16×16 WS2812B icon matrix, sharing the ESP32-C6 + LED Driver Board hardware pattern used by [Slide Score Station](../Slide%20Score%20Station/readme.md). Hardware bring-up findings are below. `main.py` runs the JSON-over-serial icon server and a `gamelib.Device` together on one panel, so icons can be authored over USB while a game runs. Icon files in `icons/` are generated by the conversion tool documented in [Image → icon conversion](#image--icon-conversion).

## Status

Hardware bring-up complete (serpentine row-major wiring confirmed, power ceilings measured). The six fruit icons in `assets/`/`icons/` are conversion-pipeline test fixtures, not vetted device content.

## Firmware layout

`hubtype.txt` reads `icon_station`; `lib/hubtype.py` holds this station's pin and geometry config, including the `max_intensity` ceiling of 0.50.

On flash, alongside the shared `Bag3/Code/lib` modules (`hubtype`, `espnow_manager`, `gamelib`, `game_store`):

| File | Role |
|---|---|
| `boot.py` | One dim pixel on GPIO0 before `main.py` imports anything. |
| `main.py` | Brings up the radio, then one `Matrix`, then `IconPanel`, `IconServer` and a `gamelib.Device`; loops over both the USB link and the radio. |
| `icon_matrix.py` | Panel geometry, the intensity clamp, and the fast path into the NeoPixel buffer. |
| `icon_store.py` | Reads and writes `icons/<name>.py`. |
| `json_link.py` | Newline-delimited JSON over stdin/stdout. |
| `icon_server.py` | The USB command set (`hello`, `list`, `show`, `save`, `frame`, `cycle`, ...). `start()`/`step()`/`finish()` let `main.py` drive it; `run()` is the standalone loop. |
| `panel.py` | `IconPanel` -- the capability handler a wand's `cap` message reaches (`icon`, `clear`, `bright`, `cycle`). |

Both `IconServer` and `IconPanel` take the same `Matrix`: one NeoPixel object per strip. A game's role file arrives in `/games/<slug>_<role>.py` and is launched when a `sys start` names its slug. `dev.on_pump` runs a non-blocking pass of the USB link on every pass of the game loop, so `list`/`show`/`save` keep working with a game running.

No card reader is fitted, so this station starts games only from the radio.
