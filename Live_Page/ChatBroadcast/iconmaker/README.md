# Icon Maker (web app), ChatBroadcast copy

**This is a copy** of `Bag3/Code/Stations/Icon Display Station/webapp/`, taken so
ChatBroadcast can open it. The two are hand-kept peers; a fix here is not a fix
there.

What this copy adds: a game-library row (`js/libraryBridge.js`) that saves an
icon into ChatBroadcast's named-icon library, where a generated display game
can refer to it by name. The Maker's own `.py` export still writes to the
downloads folder, which is what you want when authoring for a station over USB.

What it does NOT do: the serial features (`js/device/`, the device bar, the
serial monitor) talk to the **station** firmware's `icon_server.py`. The
Broadcast icon display at `Bag3/Code/BroadcastCode/BroadcastBox/IconDisplay/` does not run
that server -- it gets its icons from the Box by card tap -- so those controls
do nothing for it. They are kept because they still work against a station unit.
