...
class App {
    ...
    async handleSendMessage() {
        ...
        try {
            const result = await PyBridgeToUse.sendCommandToHub(newMessage.command);

            const isError = handleError(result, "Send Command");

            if (result.status === "sent") {
                console.log("Command sent to hub:", newMessage.command);
            } else if (isError) {
    ...
    setupClickOutsideHandler() {
        ...
        document.addEventListener('click', (event) => {
            ...
            const isClickOnOverlay = (messageDetailsOverlay && messageDetailsOverlay.contains(event.target)) ||
