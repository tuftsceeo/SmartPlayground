...
class Hub:
    ...
    def _setup_espnow(self):
        ...
        # DISABLE WiFi power save for better coexistence with BLE
        # pm=0 means WIFI_PS_NONE - keeps WiFi radio on continuously
        # This may help with ESP-NOW RX reliability when BLE is active
        try:
            sta.config(pm=0)
            print("WiFi power save DISABLED (pm=0) for better ESP-NOW reliability")
        except Exception as e:
            print("Warning: Could not disable WiFi power save:", e)
        
        # CRITICAL: Disable WiFi power save for reliable ESP-NOW RX
        # Power save mode makes WiFi sleep periodically, causing missed packets
        # WIFI_PS_NONE = 0 (no power save, max performance)
        print("Disabling WiFi power save for ESP-NOW reliability...")
        sta.config(pm=0)
        print("WiFi power save DISABLED (pm=0) - WiFi always active")
    ...
    def start_device_scan(self, rssi_threshold="all"):
        ...
        # FULL BLE STACK SHUTDOWN to free radio for ESP-NOW
        print("=== SHUTTING DOWN ENTIRE BLE STACK ===")
        try:
            # Deactivate BLE stack completely - releases radio for WiFi/ESP-NOW
            print("Calling ble._ble.active(False) to shutdown NimBLE stack...")
            self.ble._ble.active(False)
            time.sleep_ms(500)  # Allow stack to fully shutdown
            print("BLE stack DEINITIALIZED - WiFi now has 100% radio time")
            print("Radio no longer time-sliced - ESP-NOW RX should work!")
        except Exception as e:
            print("ERROR: Could not shutdown BLE stack:", e)
            print("ESP-NOW RX may still fail due to coexistence!")
    ...
    def finish_device_scan(self):
        ...
        self.ble_paused = False
        print("=== REINITIALIZING BLE STACK ===")
        try:
            # Reactivate BLE stack - restarts NimBLE and coexistence
            print("Calling ble._ble.active(True) to restart NimBLE stack...")
            self.ble._ble.active(True)
            time.sleep_ms(500)  # Allow stack to fully initialize
            print("BLE stack REINITIALIZED")
            
            # Now start advertising again
            self.ble.advertise()
            time.sleep_ms(100)
            print("BLE advertising RESUMED - radio back to time-sliced mode")
        except Exception as e:
            print("ERROR: Could not restart BLE stack:", e)
